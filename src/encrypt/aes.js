export const Aes =() => {
    const sbox =[
      
    ]
    const Rcon = [
       
    ]


    const inv_sbox = [
        
      ]


  
    let key_val = "Keys"
    let keytmp = new TextEncoder().encode(key_val)
    const Nb = 4
    const Nk = keytmp.length / 4
    const Nr = Nk + 6
    let plaintext="Hello World this is forum how are you?"
    let plaintext_bytes = new TextEncoder().encode(plaintext)
    // console.log("bytes",plaintext_bytes)
  
    // console.log("key len",keytmp.length)
    let IV = new Uint8Array(16)
    crypto.getRandomValues(IV)
    // console.log("iv",IV)
    let w = generateSubkeys(keytmp)
    // console.log("k",w.toString())
    function AddRoundKey(state,w,round){
      var tmp=new Array(state.length)
  for (var i = 0; i < state.length; i++) {
    tmp[i] = new Array(state[i].length);
  }
      for(var c=0;c<Nb;c++){
        for(var l=0;l<4;l++){
          tmp[l][c] = state[l][c] ^ w[round*Nb + c][l]
        }
      }
      return tmp
    }
    function rotateWord(inArr) {
      var tmp = new Array(inArr.length);
      tmp[0] = inArr[1];
      tmp[1] = inArr[2];
      tmp[2] = inArr[3];
      tmp[3] = inArr[0];
      return tmp;
    }

    function SubWord(inArr){
      var tmp = new Array(inArr.length);
    for (let i = 0; i < tmp.length; i++) {
      tmp[i] = sbox[inArr[i] & 0xff];
    }
    return tmp;
    }

    function SubBytes(state){
      var tmp = new Array(4)
      for(let row = 0; row<4; row++){
        tmp[row] = new Array(Nb)
        for(let col = 0; col < Nb ; col++){
          tmp[row][col] = sbox[(state[row][col] & 0xff)]
        }
      }
      return tmp
    }

    function inverseSubBytes(state){
        for(let row = 0; row<4; row++){
          for(let col = 0; col < Nb ; col++){
            state[row][col] = inv_sbox[(state[row][col] & 0xff)]
          }
        }
        return state
      }


    function ShiftRows(state){
      for(let r=1;r<4;r++){
        var t = state[r].slice(r).concat(state[r].slice(0,r))
        for(let c = 0; c<Nb;c++){
          state[r][c] = t[c]
        }
      }
      return state
    }

    function inverseShiftRows(state){
        var t = new Array(4)
        for(let r=1;r<4;r++){
          for(let c = 0; c<Nb;c++){
            t[(c+r)%Nb] = state[r][c]
            for(c=0; c<Nb; c++)
            {
                state[r][c] = t[c]
            }

          }
        }
        return state
      }


    function MixColumns(s) {
      var sp = new Array(4);
      var b02 = 0x02, b03 = 0x03;
    
      for (let c = 0; c < 4; c++) {
        sp[0] = FFMul(b02, s[0][c]) ^ FFMul(b03, s[1][c]) ^ s[2][c] ^ s[3][c];
        sp[1] = s[0][c] ^ FFMul(b02, s[1][c]) ^ FFMul(b03, s[2][c]) ^ s[3][c];
        sp[2] = s[0][c] ^ s[1][c] ^ FFMul(b02, s[2][c]) ^ FFMul(b03, s[3][c]);
        sp[3] = FFMul(b03, s[0][c]) ^ s[1][c] ^ s[2][c] ^ FFMul(b02, s[3][c]);
        for (let i = 0; i < 4; i++) s[i][c] = sp[i];
      }
    
      return s;
    }

    
    function inverseMixColumns(s) {
        var sp = new Array(4);
        var b02 = 0x0e, b03 = 0x0b;
        var b02 = 0x0e, b03 = 0x0b, b04 = 0x0d, b05 = 0x09;
      
        for (let c = 0; c < 4; c++) {
          sp[0] = FFMul(b02, s[0][c]) ^ FFMul(b03, s[1][c]) ^ s[2][c] ^ s[3][c];
          sp[1] = s[0][c] ^ FFMul(b02, s[1][c]) ^ FFMul(b03, s[2][c]) ^ s[3][c];
          sp[2] = s[0][c] ^ s[1][c] ^ FFMul(b02, s[2][c]) ^ FFMul(b03, s[3][c]);
          sp[3] = FFMul(b03, s[0][c]) ^ s[1][c] ^ s[2][c] ^ FFMul(b02, s[3][c]);
          for (let i = 0; i < 4; i++) s[i][c] = sp[i];
        }
      
        return s;
      }


    function FFMul(a, b) {
      let aa = a, bb = b, r = 0, t;
      while (aa != 0) {
        if ((aa & 1) !== 0)
          r ^= bb;
        t = bb & 0x80;
        bb <<= 1;
        if (t !== 0)
          bb ^= 0x1b;
        aa = (aa & 0xff) >>> 1;
      }
      return r;
    }
    
    function encryptBloc(inArray){
      var tmp = new Array(inArray.length)
      var state = new Array(4)
      for(var i =0 ; i<state.length;i++){
        state[i] = new Array(Nb)
      }
      for(var i=0;i<inArray.length;i++){
        state[i%4][Math.floor(i/4)] = inArray[i%4 * 4 + Math.floor(i/4)]
        // console.log(state)
      }
      state = AddRoundKey(state,w,0)
      for (var round = 1; round < Nr; round++) {
        state = SubBytes(state);
        state = ShiftRows(state);
        state = MixColumns(state);
        state = AddRoundKey(state, w, round);
        }
        
        state = SubBytes(state);
        state = ShiftRows(state);
        state = AddRoundKey(state, w, Nr);
        
        for (var i = 0; i < tmp.length; i++) {
        tmp[i % 4 * 4 + Math.floor(i / 4)] = state[Math.floor(i / 4)][i % 4];
        }
        
        return state;
    }

    // function decryptblock(inArray)
    // {
    //     var tmp = new Uint8Array(inArray.length)
    //     var state = new Array(4)
    //     for(var i =0 ; i<state.length;i++){
    //       state[i] = new Array(Nb)
    //     }
    //     for(var i=0;i<inArray.length;i++){
    //       state[i%4][Math.floor(i/4)] = inArray[i%4 * 4 + Math.floor(i/4)]
    //       // console.log(state)
    //     }
    //     state = AddRoundKey(state,w,Nr)
    //     for (var round = Nr-1; round >= 1; round--) {
    //       state = inverseSubBytes(state);
    //       state = inverseShiftRows(state);
    //       state = AddRoundKey(state, w, round);
    //       state = inverseMixColumns(state);
          
    //       }
          
    //       state = inverseSubBytes(state);
    //       state = inverseShiftRows(state);
    //       state = AddRoundKey(state, w, Nr);
          
    //       for (var i = 0; i < tmp.length; i++) {
    //       tmp[i % 4 * 4 + Math.floor(i / 4)] = state[Math.floor(i / 4)][i % 4];
    //       }
          
    //       return tmp;
    // }

    function xor_func(a,b){
      var out = new Uint8Array(a.length)
      for(let i =0 ; i<a.length;i++){
        out[i] = a[i]^b[i]
      }
      // console.log("a",a.length)
      // console.log(out);
      return out
    }


    function generateSubkeys(key) {
      var tmp = new Array(Nb * (Nr + 1)).fill().map(() => new Array(4));
      // console.log(tmp)
      // console.log(key)
      var i = 0;
      while (i < Nk) {
        tmp[i][0] = key[i * 4];
        tmp[i][1] = key[i * 4 + 1];
        tmp[i][2] = key[i * 4 + 2];
        tmp[i][3] = key[i * 4 + 3];
        i++;
      }
      i = Nk;
      while (i < Nb * (Nr + 1)) {
        var temp = new Array(4);
        for (let k = 0; k < 4; k++) {
          temp[k] = tmp[i - 1][k];
        }
        if (i % Nk === 0) {
          temp = SubWord(rotateWord(temp));
          temp[0] = temp[0] ^ (Rcon[i / Nk] & 0xff);
        } else if (Nk > 6 && i % Nk === 4) {
          temp = SubWord(temp);
        }
        tmp[i] = xor_func(tmp[i - Nk], temp);
        i++;
      }
      // console.log("tmp",tmp)
      return tmp;
    }
  

    function encryption(input,key,iv){
      let paddingLength = 0
      // let padding = new Uint8Array(1)
      let i
      paddingLength = 16 - (input.length % 16)
      // console.log(paddingLength)
      let padding = new Uint8Array(paddingLength)
      padding[0] = 0x80
      for(i =1; i < paddingLength ; i++){
        padding[i] = 0
      }
      // console.log(padding)
      var tmp = new Uint8Array(input.length + paddingLength)
      let bloc = new Uint8Array(16)
      let t1 = new Uint8Array(16)
      
      let count = 0;
      bloc = xor_func(bloc,iv)
      bloc = encryptBloc(bloc)
  
    for (i = 0; i < input.length + paddingLength; i++) {
      if (i > 0 && i % 16 === 0) {
        if (i - 16 === 0) {
          bloc = xor_func(bloc, iv);
          bloc = encryptBloc(bloc);
          // console.log("bloc",bloc)
          // tmp.set(bloc, i - 16);
          tmp = [...tmp.slice(0, i-16), ...bloc];
    // console.log("na",newArray);
          // console.log("tmp before t2",tmp)
        } else {
          let t2 = new Uint8Array(16);
          let flattmp = tmp.flat()
          t2 = [...flattmp.slice(i-16,flattmp.length)]
          // console.log("t2",t2)
          t1 = xor_func(bloc, t2);
          bloc = encryptBloc(t1);
          // console.log("bloc",bloc)
          // tmp.set(bloc, i - 16);
          tmp = [...tmp.slice(0, i-16), ...bloc];
          // console.log("tmp",tmp)
        }
      }
      if (i < input.length) {
        bloc[i % 16] = input[i];
      } else {
        bloc[i % 16] = padding[count % 16];
        count++;
      }
    }
    if (bloc.length === 16) {
      if (i - 16 === 0) {
        bloc = xor_func(bloc, iv);
        bloc = encryptBloc(bloc);
        // console.log("bloc",bloc)
        tmp = [...tmp.slice(0, i-16), ...bloc];
        // console.log("tmp",tmp)
      } else {
        let t2 = new Uint8Array(16);
        let flattmp = tmp.flat()
        t2 = [...flattmp.slice(i-16,flattmp.length)]
        // console.log("t2",t2)
        t1 = xor_func(bloc, t2);
        bloc = encryptBloc(t1);
        // console.log("bloc",bloc)
        tmp = [...tmp.slice(0, i-16), ...bloc];
        // console.log("tmp",tmp)
      }
    }
    var result = [tmp, iv];
    return tmp;
  }

  function deletePadding(input) {
    var count = 0;
    var i = input.length - 1;
    while (input[i] == 0) {
      count++;
      i--;
    }
    if (count >= input.length) {
      return new Uint8Array();
    }
    var tmp = new Uint8Array(input.length - count);
    tmp.set(input.slice(0, tmp.length));
    return tmp;
  }
  
  
  function decryptBlock(inArray) {
    var tmp = new Uint8Array(inArray.length)
    var state = new Array(4)
    for (var i = 0; i < state.length; i++) {
      state[i] = new Array(Nb)
    }
    for (var i = 0; i < inArray.length; i++) {
      state[i % 4][Math.floor(i / 4)] = inArray[i % 4 * 4 + Math.floor(i / 4)]
    }
    state = AddRoundKey(state, w, Nr)
    for (var round = Nr - 1; round >= 1; round--) {
      state = inverseSubBytes(state)
      state = inverseShiftRows(state)
      state = AddRoundKey(state, w, round)
      state = inverseMixColumns(state)
    }
    state = inverseSubBytes(state)
    state = inverseShiftRows(state)
    state = AddRoundKey(state, w, 0)
    for (var i = 0; i < tmp.length; i++) {
      tmp[i % 4 * 4 + Math.floor(i / 4)] = state[Math.floor(i / 4)][i % 4]
    }
    return tmp
  }
  
  function decryption(ct, key, iv) {
    var i = 0
    var tmp = new Uint8Array(ct.length)
    var bloc = new Uint8Array(16)
    for (i = 0; i < ct.length; i++) {
      if (i > 0 && i % 16 == 0) {
        var prevBloc = new Uint8Array(tmp.slice(i - 16, i))
        bloc = decryptBlock(bloc)
        bloc = xor_func(prevBloc, bloc)
        tmp.set(bloc, Math.max(0, i - 16))
      }
      bloc[i % 16] = ct[i]
    }
    var prevBloc = new Uint8Array(tmp.slice(i - 16, i))
    bloc = decryptBlock(bloc)
    bloc = xor_func(prevBloc, bloc)
    tmp.set(deletePadding(bloc), Math.max(0, i - 16))
    return tmp
}

  
  
  
  



  var cipher = encryption(plaintext_bytes,keytmp,IV)
  var decrypt = decryption(cipher, keytmp, IV)
  console.log(decrypt)
    console.log("cipher",cipher)
     return(
          <div>
         CBC Encryption <br/>
         Plaint Text: {plaintext} <br/>
         Bytes of Plain Text: {plaintext_bytes} <br/>
         CipherText: {cipher}
        decrypt: {decrypt}
  
          </div>
      )
  }